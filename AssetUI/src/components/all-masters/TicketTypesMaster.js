...
function TicketTypesMaster() {
    ...
    const getTicketTypeList = async () => {
        try {
            ...
            if (response.data.success) {
                setTicketTypeList(response.data.data);
            } else {
                toast.error(response.data.message || "Failed to fetch ticket types.", config.tostar_config);
            }
        } catch (error) {
        ...
        try {
            ...
            if (selectedTicketType) {
                response = await axios.post(
                    config.API_URL + `TicketMaster/UpdateTicketType?ttId=${selectedTicketType.ttId}`,
                    dataToSend
                );
    ...
    const handleEdit = async (ticketType) => {
        try {
            const response = await axios.get(
                config.API_URL + `TicketMaster/GetTicketTypeById?id=${ticketType.ttId}`
            );
            if (response.data.success) {
                setSelectedTicketType(response.data.data);
                setNewTicketType(response.data.data.ticketTypeName);
            } else {
                toast.error(response.data.message || "Failed to fetch ticket type details.", config.tostar_config);
            }
        } catch (error) {
    ...
    const confirmDelete = async () => {
        try {
            setIsLoaderActive(true);
            await axios.post(
                config.API_URL + `TicketMaster/RemoveTicketType?id=${ticketTypeToDelete.ttId}`
            );
            toast.success("Ticket type deleted successfully.", config.tostar_config);
            setTicketTypeList((prevList) =>
                prevList.filter((item) => item.ttId !== ticketTypeToDelete.ttId)
            );
